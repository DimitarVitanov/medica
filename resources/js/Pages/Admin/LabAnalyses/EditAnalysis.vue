...
const getTranslation = (translations, field, locale) => {
    ...
    const t = translations.find(tr => tr.field === field && tr.locale === locale);
    return t ? t.value : '';
};

const form = useForm({
    name: props.analysis.name || '',
    name_en: getTranslation(props.analysis.translations, 'name', 'en'),
    price: props.analysis.price || '',
    order: props.analysis.order || 0,
    is_active: props.analysis.is_active ?? true,
});

const submit = () => {
    form.put(`/admin/lab-analyses/analyses/${props.analysis.id}`);
};
</script>

<template>
    <Head title="Уреди анализа - Админ" />

    <AuthenticatedLayout>
        <template #header>
            <div class="header-wrapper">
                <div class="header-content">
                    <h1 class="page-title">Уреди анализа</h1>
                    <p class="page-subtitle">{{ analysis.category?.name }} → {{ analysis.name }}</p>
                </div>
