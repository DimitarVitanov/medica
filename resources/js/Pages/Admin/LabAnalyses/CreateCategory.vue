...
const submit = () => {
    form.post('/admin/lab-analyses/categories');
};
</script>

<template>
    <Head title="Додај категорија - Админ" />

    <AuthenticatedLayout>
        <template #header>
            <div class="header-wrapper">
                <div class="header-content">
                    <h1 class="page-title">Додај категорија</h1>
                    <p class="page-subtitle">Креирајте нова категорија за лабораториски анализи</p>
                </div>
        ...
        <div class="form-card">
            <form @submit.prevent="submit">
                <div class="form-group">
                    <label class="form-label">Име (МК) *</label>
                    <input v-model="form.name" type="text" class="form-input" placeholder="пр. Витамини" />
                    <div v-if="form.errors.name" class="form-error">{{ form.errors.name }}</div>
                </div>

                <div class="form-group">
                    <label class="form-label">Име (EN)</label>
                    <input v-model="form.name_en" type="text" class="form-input" placeholder="e.g. Vitamins" />
