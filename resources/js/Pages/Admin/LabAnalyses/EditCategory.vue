...
const form = useForm({
    ...
    name_en: getTranslation(props.category.translations, 'name', 'en'),
    order: props.category.order || 0,
    is_active: props.category.is_active ?? true,
});

const submit = () => {
    form.put(`/admin/lab-analyses/categories/${props.category.id}`);
};
</script>

<template>
    <Head title="Уреди категорија - Админ" />

    <AuthenticatedLayout>
        <template #header>
            <div class="header-wrapper">
                <div class="header-content">
                    <h1 class="page-title">Уреди категорија</h1>
